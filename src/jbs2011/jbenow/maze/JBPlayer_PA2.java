...
import jbs2011.tjhickey.maze.Item;
 ...
 *
 */
public class JBPlayer_PA2 extends MazePlayer{
	 ...
	 */
	   ...
	   public Direction nextMove(
		   ...
		   //Check to see if there's a person in surrounding area and move < 1/4 of w*d,
		   //if not, then steal from surrounding player instead (if possible)
		   if(bestMoveLength>(maze.getWidth()*maze.getDepth()/4) && !backpack.isEmpty()){
			   MazePosition oppPos = new MazePosition(playerXY.row, playerXY.col);
			   if(maze.positionPlayer.get(oppPos).size()>1){
				   int i = 0;
				   while(!maze.positionPlayer.get(oppPos).get(i).equals(this)){
					   backpack.get(0).action(players, jewels, maze, oppPos, this, 
							   					maze.positionPlayer.get(oppPos).get(i));
					   return Direction.CENTER;
				   }
			   }
			   oppPos = new MazePosition(playerXY.row+1, playerXY.col);
			   for(int i=0; i<=4; i++){
				   if(maze.positionPlayer.containsKey(oppPos)){
					   backpack.get(0).action(players, jewels, maze, oppPos, this, 
							   					maze.positionPlayer.get(oppPos).get(i));
					   return Direction.CENTER;
				   }
				   if(i == 0){
					   ...
					   oppPos = new MazePosition(playerXY.row, playerXY.col+1);
				   }
				   else if(i == 2){
					   oppPos = new MazePosition(playerXY.row, playerXY.col-1);
				   }
